...
describe('audio send result', () => {
  it('waits for the send result and preserves voice and reply options', async () => {
    ...
    let receivedOptions;
    const client = {
      sendMessage: (_target, _media, options) => {
    ...
    let completed = false;
    const promise = sendAudio(
      client,
      'test',
      { mimetype: 'audio/ogg; codecs=opus', data: Buffer.from('OggS-test').toString('base64') },
      { sendAudioAsVoice: true, quotedMessageId: 'reply' },
    ).then(() => {
      completed = true;
    });
  ...
  it('propagates send failures without retrying a potentially delivered message', async () => {
    ...
    await expect(
      sendAudio(client, 'test', { mimetype: 'audio/ogg', data: 'YWJj' }, {}),
  ...
  it('rejects invalid voice media before calling WhatsApp', async () => {
    ...
    await expect(
      sendAudio(
        client,
        'test',
        { mimetype: 'audio/webm', data: 'YWJj' },
        { sendAudioAsVoice: true },
      ),
    ).rejects.toThrow('OGG/Opus');
...
describe('published audio verification', () => {
  ...
  it('compares the remote decrypted bytes with the original and message hash', async () => {
    const client = {
      pupPage: {
        evaluate: async () => ({ status: 'downloaded', bytes: data.length, hash, ogg: true }),
      },
    };
    expect(await verifyPublishedAudio(client, sent, media)).toEqual({
      status: 'downloaded',
      bytes: data.length,
      ogg: true,
      matchesOriginal: true,
      matchesMessageHash: true,
    });
  });
  it('detects different remote content', async () => {
    const client = {
      pupPage: {
        evaluate: async () => ({ status: 'downloaded', bytes: 1, hash: 'different', ogg: false }),
      },
    };
    expect(await verifyPublishedAudio(client, sent, media)).toMatchObject({
      matchesOriginal: false,
      matchesMessageHash: false,
    });
  });
  it('reports remote failures without exposing keys or paths', async () => {
    const client = {
      pupPage: { evaluate: async () => ({ status: 'download_or_decryption_failed' }) },
    };
    expect(await verifyPublishedAudio(client, sent, media)).toEqual({
      status: 'download_or_decryption_failed',
    });
  });
});
