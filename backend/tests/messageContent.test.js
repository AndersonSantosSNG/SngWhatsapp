...
it('uses alternate LID names when the regular contact lookup fails', async () => {
    const client = { getContactById: async () => { throw Error(); }, pupPage: { evaluate: async () => [['60657856729093', 'Ana'], ['5511999999999', 'Ana']] } };
    expect(await displayMessageText({ body: 'Oi @60657856729093', mentionedIds: ['5511999999999@c.us'] }, client)).toBe('Oi @Ana');
});
it('resolves mentions in stored messages without changing the stored object', async () => {
    ...
    let calls = 0;
    const client = { getContactById: async () => ({ name: 'Ana' }), pupPage: { evaluate: async () => ++calls === 1 ? [{ id: 'm1', mentionedIds: ['123@lid'] }] : [['123', 'Ana']] } };
