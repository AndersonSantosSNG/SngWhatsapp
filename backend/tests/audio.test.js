...
describe('voice audio conversion', () => {
  it.each(['webm', 'mp4'])(
    'converts %s recordings to playable mono Opus in Ogg',
    async (format) => {
      ...
      const pcm = execFileSync(
        ffmpegPath,
        ['-hide_banner', '-loglevel', 'error', '-i', 'pipe:0', '-f', 's16le', 'pipe:1'],
        { input: output, windowsHide: true },
      );
  ...
  it('rejects empty and invalid recordings with an actionable error', async () => {
    ...
    await expect(convertVoiceAudio(Buffer.from('not audio').toString('base64'))).rejects.toThrow(
      'converter',
    );
  });
});
