  ...
  it('formata o histórico e escapa conteúdo HTML das mensagens', () => {
    const history = formatConversationHistory(
      [
        {
          sender: 'agent',
          body: 'Poderia informar o erro?',
          timestamp: '2026-09-08T14:24:00.000Z',
        },
...
describe('upload de anexos do GLPI', () => {
  it('envia o documento por multipart e o vincula ao chamado criado', async () => {
    ...
    global.fetch = vi.fn(async (url, options) => {
      ...
      const data = url.endsWith('/initSession')
    ...
    try {
      const result = await createTicket({
        ...
        attachments: [
          {
            filePath: path.join(__dirname, 'fixtures', 'anexo.txt'),
            fileName: 'anexo.txt',
            mimeType: 'text/plain',
          },
        ],
      ...
      expect(JSON.parse(link.options.body).input).toEqual({
        documents_id: 654,
        itemtype: 'Ticket',
        items_id: 321,
      });
      ...
      if (originalAppToken === undefined) delete process.env.SD_APP_TOKEN;
      else process.env.SD_APP_TOKEN = originalAppToken;
      if (originalUserToken === undefined) delete process.env.SD_USER_TOKEN;
      else process.env.SD_USER_TOKEN = originalUserToken;
    }
  });
});
