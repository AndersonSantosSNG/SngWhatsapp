...
async function glpiRequest(path, options = {}) {
  ...
  if (!appToken || !userToken)
    throw new Error('Integração com o GLPI não configurada. Informe SD_APP_TOKEN e SD_USER_TOKEN.');

  const response = await fetch(
    `${(process.env.GLPI_URL || DEFAULT_GLPI_URL).replace(/\/$/, '')}${path}`,
    {
      ...options,
      headers: {
...
async function uploadAttachment(sessionToken, ticketId, attachment) {
  const fileName = path
    .basename(attachment.fileName || attachment.filePath)
    .replace(/["\r\n]/g, '_');
  const maxBytes =
    Math.max(1, Number.parseInt(process.env.GLPI_MAX_ATTACHMENT_MB || '25', 10)) * 1024 * 1024;
  const stats = await fs.stat(attachment.filePath);
  if (stats.size > maxBytes)
    throw new Error(
      `${fileName} excede o limite local de ${Math.round(maxBytes / 1024 / 1024)} MB.`,
    );
  ...
  await glpiRequest('/Document_Item/', {
    ...
    body: JSON.stringify({
      input: { documents_id: document.id, itemtype: 'Ticket', items_id: ticketId },
    }),
  });
  ...
  try {
    ...
    const result = await glpiRequest('/Ticket/', {
      ...
      body: JSON.stringify({
        input: {
          name: title,
          content: formatConversationHistory(messages, contactName),
          urgency: 3,
          type: 1,
        },
      }),
    ...
    for (const attachment of attachments) {
      try {
        ...
        failedAttachments.push({
          fileName: path.basename(attachment.fileName || attachment.filePath),
          error: err.message,
        });
      }
    }
    ...
    if (sessionToken) {
      await glpiRequest('/killSession', { headers: { 'Session-Token': sessionToken } }).catch(
        (err) => console.error('[GLPI][ENCERRAR SESSÃO]', err.message),
      );
    }
  }
}
