...
function convertVoiceAudio(base64) {
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => finish(new Error('A conversão do áudio demorou demais. Tente uma gravação mais curta.')), 60000);
        process.on('error', () => finish(new Error('Não foi possível iniciar o conversor de áudio no servidor.')));
        process.stdin.on('error', () => {}); // FFmpeg may close stdin when the input is invalid.
        process.stderr.resume();
        process.stdout.on('data', chunk => {
            outputSize += chunk.length;
            if (outputSize > 16 * 1024 * 1024) return finish(new Error('Áudio muito grande. Envie uma gravação mais curta.'));
            chunks.push(chunk);
        });
        process.on('close', code => {
            const output = Buffer.concat(chunks);
            if (code !== 0 || output.subarray(0, 4).toString() !== 'OggS') {
                return finish(new Error('Não foi possível converter o áudio. Grave novamente ou selecione outro arquivo.'));
            }
