            ...
            const resolved = await client.pupPage.evaluate(async mentions => {
                ...
                for (const mention of mentions) {
                    ...
                    try {
                        ...
                        if (name) {
                            for (const candidate of candidates) result.push([serialize(candidate).split('@')[0], name]);
...
async function hydrateStoredMentions(messages, client) {
    ...
    const candidates = messages.filter(message => /@\d+\b/.test(message.body || '') && message.whatsappMessageId);
    if (!candidates.length) return messages;
    try {
        const metadata = await client.pupPage.evaluate(async ids => {
            const collection = window.require('WAWebCollections').Msg;
            const missing = ids.filter(id => !collection.get(id));
            let fetched = [];
            if (missing.length) {
                try { fetched = (await collection.getMessagesById(missing))?.messages || []; } catch {}
            }
            return ids.map(id => {
                const model = collection.get(id) || fetched.find(item => (item.id?._serialized || item.id?.$1) === id);
                return { id, mentionedIds: (model?.mentionedJidList || []).map(value => typeof value === 'string' ? value : value?._serialized || value?.$1).filter(Boolean) };
            });
