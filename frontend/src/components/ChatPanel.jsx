...
import { formatDateSeparator, getDateKey } from '../utils/messageDate';
...
export default function ChatPanel({
    ...
    let target = messages.find(
      (message) =>
        (quotedMessage.quotedMessageId &&
          String(message.id || message._id) === String(quotedMessage.quotedMessageId)) ||
        (quotedMessage.quotedWhatsappMessageId &&
          message.whatsappMessageId === quotedMessage.quotedWhatsappMessageId),
    );
    if (!target && quotedMessage.quotedBody) {
      const quotedIndex = messages.findIndex(
        (message) =>
          String(message.id || message._id) === String(quotedMessage.id || quotedMessage._id),
      );
  ...
  const displayName =
    chat.isGroup && genericNames.includes((chat.contactName || '').trim())
      ? 'Grupo do WhatsApp'
      : /^\+?\d+$/.test(String(rawDisplayName))
        ? formatPhone(rawDisplayName)
        : rawDisplayName;
  const whatsappId = String(chat.whatsappId || '');
  const canSendMessages =
    !whatsappId.endsWith('@newsletter') &&
    !whatsappId.endsWith('@broadcast') &&
    whatsappId !== 'status@broadcast';
  let unreadIndex = unreadMarker?.firstMessageId
    ? messages.findIndex(
        (message) => String(message.id || message._id) === String(unreadMarker.firstMessageId),
      )
    : -1;
    ...
    <section className="chat-panel">
      ...
      {closingTicketId === chat._id && (
        <CloseChatDialog
          ...
          onSend={onSend}
          onClose={onClose}
          onCancel={() => setClosingTicketId(null)}
        />
      )}
      {glpiTicketId === chat._id && (
        <OpenGlpiTicketDialog
          key={chat._id}
          chat={chat}
          onCreate={onCreateGlpiTicket}
          onCancel={() => setGlpiTicketId(null)}
        />
      )}
      {deletingMessage && (
        <DeleteMessageDialog
      ...
      <header className="chat-header">
        ...
        <div className="chat-contact">
          {chat.isGroup ? (
            <button
              ...
              onClick={() => setMembersTicketId(chat._id)}
          ...
          <div>
            <span className="chat-title-line">
              ...
              <em className={`badge ${chat.status}`}>
                {chat.status === 'closed'
                  ? 'Encerrado'
                  : chat.status === 'open'
                    ? 'Em atendimento'
        ...
        <div className="chat-actions">
          <button type="button" className="secondary" onClick={() => setGlpiTicketId(chat._id)}>
            <i className="fa-solid fa-ticket" />
            <span>Abrir chamado</span>
          </button>
          <button className={chat.status === 'open' ? 'warning' : 'primary'} onClick={onToggle}>
            <i
              className={`fa-solid ${chat.status === 'open' ? 'fa-arrow-rotate-left' : 'fa-user-check'}`}
            />
            <span>{chat.status === 'open' ? 'Devolver' : 'Assumir'}</span>
          </button>
          <button className="danger" onClick={() => setClosingTicketId(chat._id)}>
      ...
      <div className="messages" ref={messagesContainer} onScroll={handleMessagesScroll}>
        {hasOlderMessages && (
          ...
          >
            <i
              className={`fa-solid ${loadingOlderMessages ? 'fa-spinner fa-spin' : 'fa-clock-rotate-left'}`}
            />
            ...
            <Fragment key={messageId}>
              {showDateSeparator && (
                <div className="date-separator">
                  <span>{formatDateSeparator(message)}</span>
                </div>
              )}
              {index === unreadIndex && (
                <div className="unread-separator" ref={unreadSeparator}>
              ...
              <div
                ...
                className={highlightedMessage === messageId ? 'quote-highlight-wrapper' : ''}
              >
                <MessageBubble
                  message={message}
                  isGroup={chat.isGroup}
                  onImage={onOpenImage}
                  onReply={canSendMessages ? setReplyTo : undefined}
                  onEdit={canSendMessages ? startEditing : undefined}
                  onDelete={canSendMessages ? setDeletingMessage : undefined}
                  onQuotedClick={scrollToQuotedMessage}
                />
              </div>
            </Fragment>
          );
      ...
      {showScrollButton && (
        <button
          className={`scroll-to-bottom ${replyTo || editingMessage ? 'with-reply' : ''}`}
          type="button"
      ...
      {canSendMessages ? (
        <form
          className={`composer ${replyTo || editingMessage ? 'with-reply' : ''}`}
          onSubmit={submit}
        >
          {editingMessage && (
          ...
          {replyTo && (
            <div className="reply-preview">
              <div>
                <strong>
                  {(replyTo.fromMe ?? replyTo.sender === 'agent')
                    ? 'Você'
                    : replyTo.groupSenderName || replyTo.senderName || displayName}
                </strong>
                <span>{replyTo.body || (replyTo.hasMedia ? 'Mídia/Arquivo' : 'Mensagem')}</span>
              </div>
              <button
                type="button"
          ...
          >
            <i
              className={`fa-solid ${sending ? 'fa-spinner fa-spin' : editingMessage ? 'fa-check' : 'fa-paper-plane'}`}
            />
          </button>
        </form>
      ) : (
        <div className="read-only-conversation">
          ...
          <span>Esta conversa não permite o envio de mensagens.</span>
        </div>
      )}
    </section>
  );
