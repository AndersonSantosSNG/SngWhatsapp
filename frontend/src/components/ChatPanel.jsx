...
export default function ChatPanel({ ticket, messages, unreadMarker, onSend, onFile, onToggle, onClose, onOpenImage }) {
  ...
  return <section className="chat-panel">
    <header className="chat-header"><div className="chat-contact"><Avatar ticket={ticket} /><div><span className="chat-title-line"><strong>{displayName}</strong><em className={`badge ${ticket.status}`}>{ticket.status === 'closed' ? 'Encerrado' : ticket.status === 'open' ? 'Aberto' : 'Pendente'}</em></span><small>{ticket.isGroup ? 'Grupo do WhatsApp' : ticket.phoneNumber}</small></div></div><div className="chat-actions"><button className={ticket.status === 'open' ? 'warning' : 'primary'} onClick={onToggle}><i className={`fa-solid ${ticket.status === 'open' ? 'fa-arrow-rotate-left' : 'fa-user-check'}`} />{ticket.status === 'open' ? 'Devolver' : 'Assumir'}</button><button className="danger" onClick={onClose}><i className="fa-solid fa-check-double" />Encerrar</button></div></header>
    <div className="messages" onScroll={handleMessagesScroll}>{messages.map((message, index) => <Fragment key={message.id || message._id}>{index === unreadIndex && <div className="unread-separator" ref={unreadSeparator}><span>Novas mensagens</span></div>}<MessageBubble message={message} onImage={onOpenImage} /></Fragment>)}<div ref={bottom} /></div>
