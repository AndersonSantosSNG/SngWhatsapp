...
export const CLOSING_MESSAGE =
  'Mensagem automática: Este atendimento foi encerrado. Caso precise de suporte para outra solicitação, basta abrir um novo chamado em https://atendimento.sng.com.br/ ou enviar uma mensagem por aqui. Agradecemos o seu contato e estamos à disposição!';
...
export default function CloseChatDialog({ canSendMessages, onSend, onClose, onCancel }) {
  ...
  const confirm = async (event) => {
    ...
    try {
      ...
      if ((await onClose()) === false)
        throw new Error('Não foi possível encerrar o atendimento. Tente novamente.');
      onCancel();
    } catch (err) {
  ...
  return (
    <dialog
      ref={dialog}
      className="card close-chat-dialog"
    ...
    >
      <form onSubmit={confirm}>
        ...
        <label className="close-chat-option">
          <input
            type="checkbox"
            checked={sendMessage}
            disabled={closing || messageSent || !canSendMessages}
        ...
        <div className="close-chat-actions">
          ...
          <button type="submit" className="submit-button" disabled={closing}>
