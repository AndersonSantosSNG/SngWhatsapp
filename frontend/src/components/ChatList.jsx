...
export default function ChatList({
  ...
  const visibleChats = chats
    .filter((chat) => {
      ...
      if (filter === 'mine' && String(chat.assignedAgent || '') !== String(agentId || ''))
        return false;
    ...
    .sort((a, b) => {
      if (sortOrder === 'name')
        return String(a.contactName || a.phoneNumber || '').localeCompare(
          String(b.contactName || b.phoneNumber || ''),
          'pt-BR',
          { sensitivity: 'base' },
        );
  ...
  const submitNewConversation = async (event) => {
    ...
    if (localNumber.length < 10 || localNumber.length > 11)
      return setSendError('Informe o DDD e o telefone corretamente.');
    if (international && (!code || number.length > 15))
      return setSendError('Informe um código de país válido.');
    setSending(true);
    setSendError('');
    try {
  ...
  return (
    <section className="chat-column">
      ...
      <div className="conversation-toolbar">
        ...
        <details className="chat-sort-menu">
          <summary title="Ordenar conversas" aria-label="Ordenar conversas">
          ...
          <div role="menu">
            {[
              ...
              >
                <i className={`fa-solid ${sortOrder === value ? 'fa-check' : 'fa-fw'}`} />
                {label}
              </button>
            ))}
          </div>
      ...
      <div className="chat-list">
        {loading && (
          <div className="chat-loading" role="status" aria-label="Carregando conversas">
        ...
        {!loading &&
          visibleChats.map((chat) => {
            ...
            const name = chat.isGroup
              ? generic.includes((chat.contactName || '').trim())
                ? 'Grupo do WhatsApp'
                : chat.contactName
              : /^\+?\d+$/.test(String(rawName))
                ? formatPhone(rawName)
                : rawName;
            ...
            return (
              <button
                type="button"
                key={chat._id}
                className={`chat-item ${activeId === chat._id ? 'selected' : ''} ${unreadCount ? 'unread' : ''}`}
              ...
              >
                ...
                <span className="chat-content">
                  <span className="chat-top">
                    <strong>{name}</strong>
                    {unreadCount > 0 && (
                      <span className="unread-count" aria-label={`${unreadCount} mensagens novas`}>
                  ...
                  <span className="chat-meta">
                    <small>{chat.isGroup ? 'Grupo' : formatPhone(chat.phoneNumber)}</small>
                    {lastMessageTime && (
                      <time dateTime={chat.lastMessageAt}>{lastMessageTime}</time>
                    )}
                  </span>
                  <span className="last-message">
                    {chat.lastMessage === '[Mídia/Arquivo]'
                      ? '📎 Mídia'
      ...
      {showNewConversation && (
        <div
          ...
          onMouseDown={(event) => {
            if (event.target === event.currentTarget && !sending) setShowNewConversation(false);
          }}
        >
          <form className="card new-conversation-card" onSubmit={submitNewConversation}>
            <div className="new-conversation-title">
              <i className="fa-solid fa-comment-medical" />
              <div>
                <h2>Nova conversa</h2>
                <p>Informe o DDD e o telefone para abrir a conversa.</p>
              </div>
            </div>
            <label className="international-option">
              <input
                type="checkbox"
              ...
              />
              <span>É número internacional?</span>
            </label>
            {international && (
              <label>
                Código do país
            ...
            <label>
              ...
              <input
                ...
                inputMode="numeric"
                value={newPhone}
                onChange={(event) => setNewPhone(maskNationalPhone(event.target.value))}
                placeholder="(99) 99999-9999"
                required
                autoFocus={!international}
              />
            </label>
            ...
            <div className="new-conversation-actions">
              ...
              <button type="submit" className="submit-button" disabled={sending}>
                {sending ? 'Abrindo...' : 'Abrir conversa'}
              </button>
            </div>
          </form>
        </div>
