  ...
  it('envia o título e informa quantas mensagens recentes serão incluídas', async () => {
    ...
    expect(
      await screen.findByText(
        (_, element) =>
          element.classList.contains('glpi-history-summary') &&
          element.textContent.includes('1 mensagem selecionada'),
      ),
    ).toBeVisible();
    ...
    expect(
      screen.getByText(
        (_, element) =>
          element.classList.contains('glpi-history-summary') &&
          element.textContent.includes('0 arquivos selecionados'),
      ),
    ).toBeVisible();
    fireEvent.click(checkbox);
    expect(
      screen.getByText(
        (_, element) =>
          element.classList.contains('glpi-history-summary') &&
          element.textContent.includes('1 arquivo selecionado'),
      ),
    ).toBeVisible();
    fireEvent.change(screen.getByLabelText('Título do chamado'), {
      target: { value: 'Erro no sistema' },
    });
    fireEvent.submit(screen.getByLabelText('Título do chamado').closest('form'));
    await waitFor(() =>
      expect(onCreate).toHaveBeenCalledWith('Erro no sistema', [messageId], [messageId]),
    );
