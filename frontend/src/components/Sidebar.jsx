  ...
  return (
    ...
    <aside className={`sidebar ${collapsed ? 'collapsed' : ''}`}>
      <div>
        ...
        <nav>{items.map(([id, icon, label]) => <button key={id} title={label} className={tab === id ? 'active' : ''} onClick={() => setTab(id)}><i className={`fa-solid ${icon}`} />{!collapsed && <span>{label}</span>}</button>)}<button className="docs-nav-button" title="Documentação" onClick={() => window.open('/docs', '_blank', 'noopener,noreferrer')}><i className="fa-solid fa-book-open" />{!collapsed && <span>Documentação</span>}</button><button className="mobile-logout" onClick={() => setConfirmLogout(true)} title="Sair"><i className="fa-solid fa-right-from-bracket" /><span>Sair</span></button></nav>
      </div>
      <footer>
