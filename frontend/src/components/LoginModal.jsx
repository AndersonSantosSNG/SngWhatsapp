...
export default function LoginModal({ onLogin }) {
  ...
  return <div className="modal-backdrop"><form className="card login-card" onSubmit={submit}><div className="login-title"><i className="fa-solid fa-headset" /><h2>Entrar como agente</h2><p>Use seu e-mail corporativo e senha.</p></div><label>E-mail corporativo<input name="email" type="email" required autoComplete="username" /></label><label>Senha<input name="password" type="password" required autoComplete="current-password" /></label>{error && <p className="form-error">{error}</p>}<button className="submit-button" disabled={loading}>{loading ? 'Entrando...' : 'Entrar'}</button></form></div>;
}
