    ...
    <div className="ticket-list">
      ...
      {!loading && visibleTickets.map(ticket => {
        ...
        return <button type="button" key={ticket._id} className={`ticket-item ${activeId === ticket._id ? 'selected' : ''} ${unreadCount ? 'unread' : ''}`} onClick={() => onSelect(ticket)}>
          <Avatar ticket={ticket} /><span className="ticket-content"><span className="ticket-top"><strong>{name}</strong>{unreadCount > 0 && <span className="unread-count" aria-label={`${unreadCount} mensagens novas`}>{unreadCount > 99 ? '99+' : unreadCount}</span>}<em className={`badge ${ticket.status}`}>{ticket.status === 'closed' ? 'Encerrado' : ticket.status === 'open' ? 'Em atentimento' : 'Pendente'}</em></span><span className="ticket-meta"><small>{ticket.isGroup ? 'Grupo' : ticket.phoneNumber}</small>{lastMessageTime && <time dateTime={ticket.lastMessageAt}>{lastMessageTime}</time>}</span><span className="last-message">{ticket.lastMessage === '[Mídia/Arquivo]' ? '📎 Mídia' : ticket.lastMessage || 'Sem mensagens'}</span></span>
        </button>;
      })}
    </div>
