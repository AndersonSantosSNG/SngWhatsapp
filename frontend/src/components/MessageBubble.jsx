...
import { LinkifiedText, MessageMedia } from './message/MessageContent';
...
export default function MessageBubble({
  ...
  useEffect(() => {
    const sentAt = new Date(
      message.sentAt || message.timestamp || message.createdAt || 0,
    ).getTime();
  ...
  if (message.isInternalEvent) {
    const eventDate = new Date(message.timestamp || message.createdAt || Date.now()).toLocaleString(
      'pt-BR',
      { dateStyle: 'short', timeStyle: 'short' },
    );
    const icons = {
      claimed: 'fa-user-check',
      unclaimed: 'fa-arrow-rotate-left',
      closed: 'fa-circle-check',
      glpi_created: 'fa-circle-check',
      call_received: 'fa-phone-volume',
    ...
    return (
      <div
        className={`internal-event ${callEvent ? `call-event ${callFromMe ? 'call-outgoing' : 'call-incoming'}` : ''}`}
      >
        <span>
          <i
            className={`fa-solid ${callEvent ? (callFromMe ? 'fa-arrow-up' : 'fa-arrow-down') : icons[message.internalAction] || 'fa-circle-info'}`}
          />
          {callEvent && <strong className="call-origin">{callFromMe ? 'Você' : 'Contato'}</strong>}
          {glpiEvent ? (
            <>
              {message.internalActorName} abriu um chamado{' '}
              <a href={message.glpiTicketUrl} target="_blank" rel="noopener noreferrer">
                {message.glpiTicketId}
              </a>
            </>
          ) : (
            message.body
          )}
          {callEvent && ['call_missed', 'call_rejected'].includes(message.internalAction) && (
            <i
              className="fa-solid fa-phone-slash call-status-icon"
              aria-label={
                message.internalAction === 'call_missed' ? 'Chamada perdida' : 'Chamada negada'
              }
            />
          )}
          <time>{eventDate}</time>
  ...
  const encodedMedia =
    rawBody.length > 512 &&
    /^(?:\/9j\/|iVBORw0KGgo|UklGR|T2dnUw)[A-Za-z0-9+/=\r\n]+$/.test(rawBody);
  ...
  const messageAge = Number.isFinite(sentAt)
    ? Math.max(0, Date.now() - sentAt)
    : Number.POSITIVE_INFINITY;
  const canEdit =
    fromMe &&
    messageAge <= 15 * 60 * 1000 &&
    Boolean(onEdit) &&
  ...
  const canDelete =
    fromMe &&
    messageAge <= 60 * 60 * 60 * 1000 &&
    Boolean(onDelete) &&
    Boolean(message.whatsappMessageId) &&
    !message.deletedAt;
  const canCopy = Boolean(body) && !placeholder;
  const hasOwnActions = canEdit || canDelete;
  const hasMenuActions = Boolean(onReply) || canCopy || hasOwnActions;
  ...
  return (
    ...
    >
      ...
      <div className="message-actions" ref={menuRef}>
        ...
        {menuOpen && (
          <div className={`message-context-menu ${menuBelow ? 'open-below' : ''}`} role="menu">
            {onReply && (
              <button
                type="button"
                role="menuitem"
      ...
      <div className="message-bubble" style={{ transform: `translateX(${swipeOffset}px)` }}>
        {message.quotedBody && (
          <button
            type="button"
            className="quoted-message"
            onClick={() => onQuotedClick?.(message)}
            title="Ir para a mensagem citada"
            aria-label="Ir para a mensagem citada"
          >
            <strong>{message.quotedSenderName || 'Mensagem'}</strong>
            <span>{message.quotedBody}</span>
          </button>
        )}
        ...
        <span className="message-time">
          ...
          {message.deletedAt && (
            <em className="message-deleted">
              <i className="fa-solid fa-ban" /> apagada
            </em>
          )}
          {time}
          {fromMe && (
