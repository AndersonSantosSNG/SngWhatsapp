...
export default function App() {
  ...
  const syncChat = useCallback((ticket) => {
    ...
    setActiveChat((current) =>
      String(current?._id || '') === ticketId ? { ...current, ...ticket } : current,
    );
  }, []);
  ...
  const selectChat = async (ticket) => {
    ...
    setMessages(
      result.data.map((message) => ({
        ...message,
        mediaUrl: message.hasMedia ? `/api/messages/${message._id}/media` : message.mediaUrl,
      })),
    );
  ...
  const loadOlderMessages = async () => {
    ...
    try {
      ...
      const result = await api(
        `/tickets/${activeChat._id}/messages?limit=100&before=${encodeURIComponent(oldest)}`,
      );
      const older = result.data.map((message) => ({
        ...message,
        mediaUrl: message.hasMedia ? `/api/messages/${message._id}/media` : message.mediaUrl,
      }));
      setMessages((current) => [
        ...older.filter(
          (item) =>
            !current.some((saved) => String(saved.id || saved._id) === String(item.id || item._id)),
        ),
        ...current,
      ]);
      setHasOlderMessages(Boolean(result.meta?.hasMore));
  ...
  useEffect(() => {
    ...
    const loadPresence = async () => {
      try {
        ...
        const result = await api(
          `/whatsapp/presence?contactId=${encodeURIComponent(contactId)}&phoneNumber=${encodeURIComponent(activeChat.phoneNumber || '')}`,
        );
    ...
    activeChat?.whatsappId,
  ]);

  const login = async (corporateEmail, password) => {
  ...
  const send = (message, replyToMessageId, { isClosingMessage = false } = {}) =>
    sendMessage({ number: activeChat.phoneNumber, message, replyToMessageId, isClosingMessage });
  ...
  const startConversation = async (number) => {
    const result = await api('/tickets/start', {
      method: 'POST',
  ...
  const updateChat = async (action) => {
    try {
      const result = await api(`/tickets/${action}`, {
        method: 'POST',
        body: JSON.stringify({ ticketId: activeChat._id }),
      });
      syncChat(result.data);
      showToast(result.message || 'Atendimento atualizado.');
      return true;
    } catch (err) {
  ...
  const close = async () => {
    const closed = await updateChat('close');
    if (closed) {
      setActiveChat(null);
      setMessages([]);
    }
  ...
  const createGlpiTicket = async (title, messageIds, attachmentMessageIds) => {
    const result = await api(`/tickets/${activeChat._id}/glpi`, {
      method: 'POST',
      body: JSON.stringify({ title, messageIds, attachmentMessageIds }),
    });
  ...
  const totalUnread = Object.values(unreadByChat).reduce(
    (total, unread) => total + unread.count,
    0,
  );
  useEffect(() => {
  ...
  return (
    <div
      className={`app-shell ${agent && tab === 'chats' && activeChat ? 'mobile-chat-open' : ''}`}
    >
      {agent && <Sidebar {...{ tab, setTab, agent, connected, collapsed, setCollapsed, logout }} />}
      {agent && tab === 'chats' && (
        <main className={`conversations-layout ${activeChat ? 'has-active-chat' : ''}`}>
          <ChatList
            {...{
              chats: chats,
